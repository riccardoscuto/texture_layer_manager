...
def _detect_pbr_channel(filename):
    ...
    stem = _os.path.splitext(name_lower)[0]
...
class TLM_OT_ImportPBRSet(Operator):
    ...
    def execute(self, context):
        ...
        for ch_id, fpath in detected.items():
            self.report({'INFO'}, f"  {ch_id}: {_os.path.basename(fpath)}")
        ...
        name = _os.path.basename(_os.path.normpath(directory)) or "PBR Import"
        ...
        for ch_id, fpath in detected.items():
            try:
                ...
                self.report({'WARNING'}, f"Failed to load {_os.path.basename(fpath)}: {e}")
