...
def _apply_adjustment(node_tree, layer, current_output, x, y, channel_id="base_color"):
    ...
    elif adj == 'GRADIENT_MAP':
        # Recolour the layers below by luminance through the layer's Color
        # Ramp (reuses the procedural ramp: Color1..Color2(+Color3), contrast,
        # center, manual / multi stops). A Photoshop-style gradient map.
        from .procedurals import _build_proc_color_ramp
        bw = node_tree.nodes.new("ShaderNodeRGBToBW")
        bw.name = f"{TLM_PREFIX}adj_gm_bw_{_next_id()}"
        bw.label = "Gradient Map Luma"
        bw.location = (x, y)
        _tag(bw, layer.name, "adj_gm_bw")
        node_tree.links.new(current_output, bw.inputs["Color"])
        mapped = _build_proc_color_ramp(node_tree, layer, x + 220, y, bw.outputs["Val"])
        return _wrap_adjustment_opacity(node_tree, layer,
                                        current_output, mapped, x, y,
                                        channel_id=channel_id)
